...
const enterAnimation = (container) => {
  return gsap.from(container, {autoAlpha: 0, clearProps:"all", duration: .6, ease: "none"})
}
const leaveAnimation = (container) => {
  return gsap.to(container, {autoAlpha: 0, clearProps:"all",  duration: .6, ease: "none" })
}
const buttonLeaveAnimation = () => {
  var tl = gsap.timeline()
  tl.to("button", {transform: "translate(0px, 0px)", duration: .8})
  
}
const buttonEnterAnimation = () => {
 gsap.to("button", {transform: "translate(0px, -300px)", opacity: 1, delay:1.1, duration: .8})
}
